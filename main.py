...
@app.get("/srcpositions/{book_id}", response_model=List[schemas.File])
...
@app.get("/books", response_model=List[schemas.Filelist])
def books_action_list(limit: int = 1000, offset: int = 0, db: Session = Depends(get_db)):
...
@app.get("/books/{book_id}/{user_id}", response_model=List[Union[schemas.Item]])
